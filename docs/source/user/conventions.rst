...
* **Polarization and powder averaging** are given as a list of channels. For XAS
  each entry is ``(kind, angle)`` with ``kind`` one of ``'linear'``,
  ``'circular'`` or ``'powder'``. For RIXS each entry is a 4-tuple
